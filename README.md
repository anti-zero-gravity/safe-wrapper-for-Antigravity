...
> **AIエージェントの挙動に関する注意**
> 
> AI（Antigravity等）は、タスクを完遂するために柔軟な手段を選択するよう設計されています。
> もし `Remove-Item` がダイアログ待ちでブロックされたと判断した場合、**AIは `git rm` やその他のバイナリツールを使用して削除を試みる**可能性があります。
> 本ツールはあくまで「PowerShell標準コマンドレットによる不慮の事故」を防ぐものであり、AIのあらゆる削除動作を物理的に封じるものではない点にご留意ください。
