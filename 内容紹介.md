# 🚫 絶対禁止コマンド（自動Accept設定によらずサイレントで禁止）
以下の破壊的コマンドは、リスク管理のため、AIによる実行を原則禁止とします。
（現状の実装では機能的にブロックされていませんが、運用ルールとして禁止します）

### 【優先度：高】ディスク・パーティション操作（データ消失リスク大）
* `Clear-Disk` : ディスクのパーティション情報を完全に消去
